...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// 加载本地类 确保 bean 的 class 已经被加载进来
		Class<?> resolvedClass = resolveBeanClass(mbd, beanName);
		if (resolvedClass != null && !mbd.hasBeanClass() && mbd.getBeanClassName() != null) {
			// TODO: 2022/9/20 此处 new RootBeanDefinition(mbd);
		...
		try {
			 ...
			 * 实例化前
			 * 处理一些实例化前的方法
			 * 此时没有实例化，如果返回对象（在实例化前方法中创建了）则直接return
			 * return的对象类型没有任何限制
 			 */
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// 先在 this.factoryBeanInstanceCache 缓存中获取，如果获取不到再去创建
		// 此处缓存为了解决 依赖注入、DependsOn等已经提前创建好的 Bean ， 避免二次重复创建
		if (mbd.isSingleton()) {
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			// 判断当前 Bean 是否合成的 并且 是否有实现了实例化前的接口的类
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				// 获取当前 Bean 的类型
				Class<?> targetType = determineTargetType(beanName, mbd);
				if (targetType != null) {
					// 遍历实现了实例化前的接口的类（需注意此处只返回一个结果并非返回集合）
					bean = applyBeanPostProcessorsBeforeInstantiation(targetType, beanName);
					// TODO: 2022/9/22 实例化前创建 Bean 则直接掉用 初始化后（applyBeanPostProcessorsAfterInitialization）
					if (bean != null) {
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		// 获取当前mbd的 supplier 如果有 则使用的 supplier 去创建对象
		 ...
		 */
 		if (resolved) {
			...
			if (autowireNecessary) {
				...
				// 如果不需要注入，则表示用的是默认的无参构造方法，则直接调用实例化
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		// 属性填充过程中， 判断 Bean 的实例化后置处理器（流程：实例化后，属性注入前）
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			for (InstantiationAwareBeanPostProcessor bp : getBeanPostProcessorCache().instantiationAware) {
				// 调用实例化后方法
		...
		// 判断 mbd 是否有 hasPropertyValues()
		PropertyValues pvs = (mbd.hasPropertyValues() ? mbd.getPropertyValues() : null);

		// Spring 自带的依赖注入
		// @Bean (autuwire = AutoWired.BY_NAME) || autowire="byName"（XML文件） 方式可以注入
		// 原理：会去寻找当前类里面所有的set方法， 然后根据 BY_NAME 或者 BY_TYPE 的其中一种方式注入对应的类（BY_NAME 方式用的时 set方法名 去掉set前缀作为名字进行寻找）
		// update: @Bean (autuwire = AutoWired.BY_NAME) 注解已经不可使用   XML方式还来可以使用（autowire="byName"） 本方式不会处理基本类型的注入
		...
		// 判断 hasInstantiationAwareBeanPostProcessors() 是否有实例化后置处理器 后执行 postProcessProperties() 方法来处理我们添加的注解注入（@AutoWiring）
		boolean hasInstAwareBpps = hasInstantiationAwareBeanPostProcessors();
		boolean needsDepCheck = (mbd.getDependencyCheck() != AbstractBeanDefinition.DEPENDENCY_CHECK_NONE);

		if (hasInstAwareBpps) {
			// pvs 为后置处理器中一个方法可以添加 PropertyValues 此时如果主动添加了属性，Spring 便会使用手动在后置处理器中添加属性注入
			if (pvs == null) {
	...
	protected void autowireByName(
		...
		// 获取当前 BeanWrapper 的需要设置的属性名字   包括set方法名
		String[] propertyNames = unsatisfiedNonSimpleProperties(mbd, bw);
		for (String propertyName : propertyNames) {
			if (containsBean(propertyName)) {
				Object bean = getBean(propertyName);
				// 自带的注入，这儿只是添加到 pvs 中，并没有直接 set 进属性
				pvs.add(propertyName, bean);
				// 记录一下当前 beanName 依赖了 propertyName
		...
		for (String propertyName : propertyNames) {
			try {
				...
				if (Object.class != pd.getPropertyType()) {
					...
					// 对象实现了 PriorityOrdered 接口表示立即初始化当前 Bean ，优先级最高
					boolean eager = !(bw.getWrappedInstance() instanceof PriorityOrdered);
					DependencyDescriptor desc = new AutowireByTypeDependencyDescriptor(methodParam, eager);
					// resolveDependency 更具类型找到想要的结果
	...
	protected String[] unsatisfiedNonSimpleProperties(AbstractBeanDefinition mbd, BeanWrapper bw) {
		...
		for (PropertyDescriptor pd : pds) {
			...
			if (pd.getWriteMethod() != null
					...
					// 4.属性类型不是简单类型，比如 int、Integer、 int[]
