...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		/**
		 * transformedBeanName(name) 获取真实beanName
		 * 1.此方法去除&前缀
		 * 2.去 aliasMap 中查找别名 直到没有返回值时才能确定beanName不是别名
		 * aliasMap
		 */
		...
		if (sharedInstance != null && args == null) {
			...
			try {
				...
				/**
				 * 解决Bean的 dependsOn 关系
				 * 利用 dependentBeanMap 解决
				 * 对于出现依赖的类去dependentBeanMap查询对应的关系
				 * 如果有，则失败
				 * 否则 dependsOn的beanName作为key，当前bean作为value保存在 dependentBeanMap 中
				 * 当执行到另一bean时  可以在 dependentBeanMap 中查询到对应的关系，则直接报错为循环依赖
				 * eg:
				 * A.dependsOn("B")
				 * dependentBeanMap.get("A") == null
				 * dependentBeanMap.add(new Set< >("B","A"))
				 * dependentBeanMap.get("B") != null
	...
	public boolean isSingleton(String name) throws NoSuchBeanDefinitionException {
		...
		// 判断实例是否继承 FactoryBean 类， 如果继承 返回FactoryBean.isSingleton()方法或者判断是否BeanFactory中的Bean名字
		...
		// 1判断是否单例 2判断是否 FactoryBean 产生的特殊 Bean
		if (mbd.isSingleton()) {
	...
	protected Object getObjectForBeanInstance(
		...
		/**
		 * BeanFactoryUtils.isFactoryDereference(name) 判断是否是FactoryBean
		 * 1.判断beanName是否是&开头的
		 * 2.&符号开头的Bean如果没有实现FactoryBean接口直接报错
		 */
		if (BeanFactoryUtils.isFactoryDereference(name)) {
